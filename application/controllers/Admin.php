...
class Admin extends CI_Controller {
	...
	public function pegawai_delete($id)
	{
		...
		$this->M_data->delete_pegawai($id);
		$this->db->trans_complete();
		$this->session->set_flashdata('message', 'swal("Berhasil!", "Delete Data Pegawai", "success");');
		redirect('admin/pegawai');
	}
	public function reset_pass($nip){
		$user = [
			'password'	=> md5('12345'),
		];
		$this->db->update('user',$user,['nip'=>$nip]);
		$this->session->set_flashdata('message', 'swal("Berhasil!", "Reset Password", "success");');
		redirect('admin/pegawai');
	}
	//end CURD pegawai
	//Data Absensi
	...
	public function view_bukti($id){
		$data['web']	= $this->web;
		$data['title']	= 'View Bukti';
		$data['data']	= $this->db->get_where('cuti',['id_cuti'=>$id])->row();
		$data['body']	= 'pegawai/view_bukti';
		$this->load->view('template',$data);
	}
	...
	public function view_data($id){
		$data['web']	= $this->web;
		$data['data']	= $this->db->query("SELECT cuti.*, user.nama, departemen.departemen, user.email from cuti left join pegawai on pegawai.nip=cuti.nip left join user on user.nip=pegawai.nip left join departemen on departemen.departemen_id=pegawai.id_departemen where cuti.nip = '$id'")->result();
		// $data['detail']	= $this->M_data->pegawaiid($id)->row();
		$data['title']	= 'Detail Cuti Pegawai';
		$data['body']	= 'admin/pegawai_cuti';
		$this->load->view('template',$data);
	}
	//laporan bulanan
	function laporan(){
